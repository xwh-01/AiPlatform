...
const ragFallbackNotice = "提示：当前没有成功检索到可用知识库内容，下面回答未使用 RAG 知识库。\n\n"

const ragQueryRewritePrompt = `Rewrite the current user question into a standalone search query for knowledge-base retrieval.
Use the conversation history only to resolve references such as "it", "this", "that", or follow-up questions.
...
func (o *AliRAGModel) GenerateResponse(ctx context.Context, messages []*schema.Message) (*schema.Message, error) {
	...
	if err != nil {
		...
		resp.Content = ragFallbackNotice + resp.Content
		return resp, nil
	}
	...
	searchQuery := o.rewriteRAGQuery(ctx, messages, query)
	...
	// 4. 构建包含检索结果的提示词
	if len(docs) == 0 {
		log.Printf("RAG retrieve returned no documents")
		resp, err := o.llm.Generate(ctx, messages)
		if err != nil {
			return nil, fmt.Errorf("ali rag generate failed: %v", err)
		}
	...
	ragPrompt := rag.BuildRAGPrompt(searchQuery, docs)
...
func (o *AliRAGModel) StreamResponse(ctx context.Context, messages []*schema.Message, cb StreamCallback) (string, error) {
	...
	if err != nil {
		log.Printf("Failed to create RAG query (user may not have uploaded file): %v", err)
		// 如果用户没有上传文件，直接使用原始问题
		return o.streamWithoutRAGNotice(ctx, messages, cb)
	}
...
func (o *AliRAGModel) streamWithoutRAGNotice(ctx context.Context, messages []*schema.Message, cb StreamCallback) (string, error) {
	cb(ragFallbackNotice)
	content, err := o.streamWithoutRAG(ctx, messages, cb)
	if err != nil {
		return "", err
	}
	return ragFallbackNotice + content, nil
}

func (o *AliRAGModel) rewriteRAGQuery(ctx context.Context, messages []*schema.Message, query string) string {
	if !shouldRewriteQuery(query, messages) {
		return query
	}

	history := formatRecentHistory(messages, 6)
	if history == "" {
		return query
	}

	prompt := fmt.Sprintf(ragQueryRewritePrompt, history, query)
	...
	if err != nil {
		log.Printf("Failed to rewrite RAG query: %v", err)
		return query
	}
...
func shouldRewriteQuery(query string, messages []*schema.Message) bool {
	if len(messages) <= 1 {
		return false
	}
	...
	followUpWords := []string{
		...
		"继续", "具体", "详细", "为什么", "怎么实现",
		"怎么做", "什么意思", "然后呢",
		"it", "this", "that", "above", "previous", "continue",
	}
	for _, word := range followUpWords {
...
func formatRecentHistory(messages []*schema.Message, maxMessages int) string {
	if len(messages) <= 1 {
		return ""
	}
