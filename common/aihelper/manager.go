...
func (m *AIHelperManager) GetOrCreateOrSwitchAIHelper(userName string, sessionID string, modelType string, config map[string]interface{}) (*AIHelper, error) {
	helper, err := m.GetOrCreateAIHelper(userName, sessionID, modelType, config)
	if err != nil {
		return nil, err
	}
	...
	model, err := GetGlobalFactory().CreateAIModel(ctx, modelType, config)
	if err != nil {
		return nil, err
	}
	helper.SwitchModel(model)
	return helper, nil
}
